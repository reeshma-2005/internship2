...
app.post("/getadjustmentdata",async(req,res)=>{
  try{
    const getAdjustment=await AdjustmentModel.find();
    console.log(getAdjustment)
    // res.status(201).json({status:201,getItem})
    res.send(getAdjustment)
  }catch(error){
    res.status(401).json({status:401,error})
  }
})
...
app.post("/purchaseorder", async (req, res) => {
  const { customername, itemname, squantity, shipcost, podate, status } =
    req.body;
  ...
  try {
    ...
    if (savepurchaseorder) {
      res.status(201).send({ message: "Purchase order added" });
    } else {
      res.status(500).send({ error: "Failed to add" });
    }
  } catch (err) {
...
app.post("/getpurchaseorderdata",async(req,res)=>{
  try{
    const getPurchase=await PurchaseorderModel.find();
